...
describe('[Feed]: fetchPackage', () => {
  ...
  it('should fetch registry data for package', done => {
    baseTest({ scope: 'scope', name: 'package', version: '1.0.0' }, { json: true }, false).then(
      ({ cached, version }) => {
  ...
  it('should fetch registry data for package if version is missing', done => {
    baseTest({ scope: 'scope', name: 'package', version: undefined }, { json: true }, false).then(
      ({ cached, version }) => {
        expect(version).toBe('1.0.0');
        expect(cached).toBeFalsy();
        done();
  ...
  it('should not fetch registry data if package is cached', done => {
    baseTest({ scope: 'scope', name: 'package', version: '1.0.0' }, { json: true }, true).then(
      ({ cached, version }) => {
        expect(version).toBe('1.0.0');
  ...
  it('should fail to get version that does not exist', done => {
    ...
    baseTest({ scope: 'scope', name: 'package', version: '1.0.0' }, { json: true }, false).catch(err => {
