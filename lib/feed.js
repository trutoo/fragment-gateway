  ...
  return new Promise((resolve, reject) => {
    request.get(
      ...
      (err, res, regData) => {
        ...
        // Update find latest version that satisfies range
        const range = !version || version == 'latest' ? '*' : version;
        version = semver.maxSatisfying(Object.keys(regData.versions), range);

        // Handle when version doesn't not satisfy range
        if (!version) {
          logger.debug(`Version range [${range}] does not satisfy any packages in registry [${config.registry}]`);
          const err = new Error(`Failed to retrieve package with version satisfying range [${range}]`);
          err.code = '404';
          return reject(err);
        }
